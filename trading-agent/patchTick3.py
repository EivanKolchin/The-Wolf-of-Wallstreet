...
part1, sep, part2 = text.partition("const wsUrl = \nwss://stream.binance.com:9443/ws/@kline_;")

part2_1, sep2, part2_2 = part2.partition("if (seriesRef.current) {")
part2_2_1, sep3, part2_2_2 = part2_2.partition("seriesRef.current.update(newCandle as any);")
part2_2_2_1, sep4, part2_2_2_2 = part2_2_2.partition("}")
...
text = re.sub(r'const wsUrl =\s+wss://stream\.binance\.com:9443/ws/\$\{symbol\.toLowerCase\(\)\.replace\(/\[\^a-z0-9\]/g,\s+\'\'\)\}@kline_\$\{fetchTimeframe\.toLowerCase\(\)\};.*?if \(seriesRef\.current\) \{\s*seriesRef\.current\.update\(newCandle as any\);\s*\}', new_code, text, flags=re.DOTALL)
